...
;
CREATE INDEX localisedblob_t_basket_idx ON arp_npl_oereb.localisedblob ( t_basket );
CREATE INDEX localisedblob_t_datasetname_idx ON arp_npl_oereb.localisedblob ( t_datasetname );
CREATE INDEX localisedblob_multilingualblob_lclsdblob_idx ON arp_npl_oereb.localisedblob ( multilingualblob_localisedblob );
...
CREATE TABLE arp_npl_oereb.multilingualblob (
  T_Id bigint PRIMARY KEY DEFAULT nextval('arp_npl_oereb.t_ili2db_seq')
  ,T_basket bigint NOT NULL
  ,T_datasetname varchar(200) NOT NULL
  ,T_Seq bigint NULL
...
CREATE INDEX multilingualuri_amt_amt_amtimweb_idx ON arp_npl_oereb.multilingualuri ( amt_amt_amtimweb );
...
CREATE INDEX multilingualuri_transfrstrkstsdnst_vrwswms_idx ON arp_npl_oereb.multilingualuri ( transfrstrkstllngsdnst_verweiswms );
COMMENT ON COLUMN arp_npl_oereb.multilingualuri.amt_amt_amtimweb IS 'Verweis auf die Website des Amtes z.B. "http://www.jgk.be.ch/jgk/de/index/direktion/organisation/agr.html".';
...
COMMENT ON COLUMN arp_npl_oereb.multilingualuri.transfrstrkstllngsdnst_verweiswms IS 'WMS GetMap-Request (für Maschine-Maschine-Kommunikation) inkl. alle benötigten Parameter, z.B. "https://wms.geo.admin.ch/?SERVICE=WMS&REQUEST=GetMap&VERSION=1.1.1&STYLES=default&SRS=EPSG:21781&BBOX=475000,60000,845000,310000&WIDTH=740&HEIGHT=500&FORMAT=image/png&LAYERS=ch.bazl.kataster-belasteter-standorte-zivilflugplaetze.oereb"';
-- OeREBKRM_V2_0.ThemaRef
CREATE TABLE arp_npl_oereb.themaref (
  T_Id bigint PRIMARY KEY DEFAULT nextval('arp_npl_oereb.t_ili2db_seq')
  ,T_basket bigint NOT NULL
  ,T_datasetname varchar(200) NOT NULL
  ,T_Seq bigint NULL
...
;
CREATE INDEX amt_amt_t_basket_idx ON arp_npl_oereb.amt_amt ( t_basket );
CREATE INDEX amt_amt_t_datasetname_idx ON arp_npl_oereb.amt_amt ( t_datasetname );
COMMENT ON TABLE arp_npl_oereb.amt_amt IS 'Eine organisatorische Einheit innerhalb der öffentlichen Verwaltung, z.B. eine für Geobasisdaten zuständige Stelle.';
COMMENT ON COLUMN arp_npl_oereb.amt_amt.auid IS 'UID der organisatorischen Einheit';
...
COMMENT ON COLUMN arp_npl_oereb.vorschriften_dokument.nuringemeinde IS 'Falls das Dokument nur eine bestimmte Gemeinde betrifft (z.B. Sicherheitszonenplan)';
COMMENT ON COLUMN arp_npl_oereb.vorschriften_dokument.rechtsstatus IS 'Status, ob dieses Element in Kraft ist';
COMMENT ON COLUMN arp_npl_oereb.vorschriften_dokument.publiziertab IS 'Datum, ab dem dieses Dokument in Auszügen erscheint';
COMMENT ON COLUMN arp_npl_oereb.vorschriften_dokument.publiziertbis IS 'Datum, an/ab dem dieses Dokument nicht mehr in Auszügen erscheint';
...
CREATE INDEX transfrstrktrtmsbschrnkung_legende_idx ON arp_npl_oereb.transferstruktur_eigentumsbeschraenkung ( legende );
...
COMMENT ON COLUMN arp_npl_oereb.transferstruktur_eigentumsbeschraenkung.publiziertbis IS 'Datum, an/ab dem diese Eigentumsbeschränkung nicht mehr in Auszügen erscheint';
...
COMMENT ON COLUMN arp_npl_oereb.transferstruktur_geometrie.punkt_lv95 IS 'Punktgeometrie';
COMMENT ON COLUMN arp_npl_oereb.transferstruktur_geometrie.linie_lv95 IS 'Linienförmige Geometrie';
COMMENT ON COLUMN arp_npl_oereb.transferstruktur_geometrie.flaeche_lv95 IS 'Flächenförmige Geometrie';
...
COMMENT ON COLUMN arp_npl_oereb.transferstruktur_geometrie.publiziertbis IS 'Datum, an/ab dem diese Geometrie nicht mehr in Auszügen erscheint';
...
CREATE INDEX transferstruktur_lgndntrag_darstellungsdienst_idx ON arp_npl_oereb.transferstruktur_legendeeintrag ( darstellungsdienst );
COMMENT ON TABLE arp_npl_oereb.transferstruktur_legendeeintrag IS 'Ein Eintrag in der Planlegende.';
COMMENT ON COLUMN arp_npl_oereb.transferstruktur_legendeeintrag.symbolpunkt IS 'Grafischer Teil des Legendeneintrages für die Darstellung. Im PNG-Format mit 300dpi oder im SVG-Format';
...
ALTER TABLE arp_npl_oereb.vorschriften_dokument ADD CONSTRAINT vorschriften_dokument_nuringemeinde_check CHECK( nuringemeinde BETWEEN 1 AND 9999);
ALTER TABLE arp_npl_oereb.vorschriften_dokument ADD CONSTRAINT vorschriften_dokument_zustaendigestelle_fkey FOREIGN KEY ( zustaendigestelle ) REFERENCES arp_npl_oereb.amt_amt DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE arp_npl_oereb.transferstruktur_eigentumsbeschraenkung ADD CONSTRAINT transfrstrktrtmsbschrnkung_legende_fkey FOREIGN KEY ( legende ) REFERENCES arp_npl_oereb.transferstruktur_legendeeintrag DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE arp_npl_oereb.transferstruktur_eigentumsbeschraenkung ADD CONSTRAINT transfrstrktrtmsbschrnkung_zustaendigestelle_fkey FOREIGN KEY ( zustaendigestelle ) REFERENCES arp_npl_oereb.amt_amt DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE arp_npl_oereb.transferstruktur_geometrie ADD CONSTRAINT transferstruktur_geometrie_zustaendigestelle_fkey FOREIGN KEY ( zustaendigestelle ) REFERENCES arp_npl_oereb.amt_amt DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE arp_npl_oereb.transferstruktur_hinweisdefinition ADD CONSTRAINT transferstrktr_hnwsdfntion_zustaendigestelle_fkey FOREIGN KEY ( zustaendigestelle ) REFERENCES arp_npl_oereb.amt_amt DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE arp_npl_oereb.transferstruktur_legendeeintrag ADD CONSTRAINT transferstruktur_lgndntrag_darstellungsdienst_fkey FOREIGN KEY ( darstellungsdienst ) REFERENCES arp_npl_oereb.transferstruktur_darstellungsdienst DEFERRABLE INITIALLY DEFERRED;
...
INSERT INTO arp_npl_oereb.T_ILI2DB_TRAFO (iliname,tag,setting) VALUES ('OeREBKRMtrsfr_V2_0.Transferstruktur.DarstellungsDienstEigentumsbeschraenkung','ch.ehi.ili2db.inheritance','embedded');
...
INSERT INTO arp_npl_oereb.T_ILI2DB_COLUMN_PROP (tablename,subtype,columnname,tag,setting) VALUES ('transferstruktur_eigentumsbeschraenkung',NULL,'zustaendigestelle','ch.ehi.ili2db.foreignKey','amt_amt');
...
INSERT INTO arp_npl_oereb.T_ILI2DB_COLUMN_PROP (tablename,subtype,columnname,tag,setting) VALUES ('transferstruktur_geometrie',NULL,'zustaendigestelle','ch.ehi.ili2db.foreignKey','amt_amt');
INSERT INTO arp_npl_oereb.T_ILI2DB_COLUMN_PROP (tablename,subtype,columnname,tag,setting) VALUES ('transferstruktur_legendeeintrag',NULL,'darstellungsdienst','ch.ehi.ili2db.foreignKey','transferstruktur_darstellungsdienst');
...
INSERT INTO arp_npl_oereb.T_ILI2DB_SETTINGS (tag,setting) VALUES ('ch.interlis.ili2c.ilidirs','models/;http://models.geo.admin.ch;https://geo.so.ch/models');
...
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('OeREBKRM_V2_0','technicalContact','mailto:vermessung@swisstopo.ch');
...
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('AdministrativeUnitsCH_V1','furtherInformation','https://www.geo.admin.ch/de/geoinformation-schweiz/geobasisdaten/geodata-models.html');
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('AdministrativeUnitsCH_V1','technicalContact','mailto:models@geo.admin.ch');
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('OeREBKRMvs_V2_0','furtherInformation','https://www.cadastre.ch/oereb-public');
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('OeREBKRMvs_V2_0','technicalContact','mailto:vermessung@swisstopo.ch');
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('OeREBKRMtrsfr_V2_0','furtherInformation','https://www.cadastre.ch/oereb-public');
INSERT INTO arp_npl_oereb.T_ILI2DB_META_ATTRS (ilielement,attr_name,attr_value) VALUES ('OeREBKRMtrsfr_V2_0','technicalContact','mailto:vermessung@swisstopo.ch');
